...
function updateThemeWithAppSkinInfo(appSkinInfo) {
    ...
    if(appName == "PHXS"){
	    addRule(styleId, "button, select, input[type=button], input[type=number], input[type=submit]", "border-radius:2px;");
	}
	if(appName == "PHXS" || appName == "PPRO" || appName == "PRLD") {
	    ...
	    addRule(styleId, "body, button, select, input[type=text], input[type=submit], input[type=number]", "background-color:" + toHex(panelBackgroundColor) + ";");
